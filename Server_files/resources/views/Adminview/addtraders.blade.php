...
<div class="page-content">
   <div class="container-fluid">
               ...
               <div class="card-body">
                    ...
                    <div class="form-row">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                <label class="text-secondary">Telegram Link</label>
                                <input type="text" name="tgLink" class="form-control" placeholder="Telegram link" required>
                            </div>
                        </div>
                    </div>
      ...
      <table class="table">
        ...
        <tbody>
           @foreach ($all_traders as $Traders)
          <tr>
             <td>{{ $Traders->tradersname }}</td>
             <td>{{ $Traders->return_rate }}</td>
             <td>{{ number_format($Traders->followers) }}</td>
             <td>{{ $Traders->profitshare }}</td>
             <td>
            <a href="{{ $Traders->tgLink }}" target="_blank">{{ $Traders->tgLink }}</a>
             </td>
             <td>
               <form action="{{ route('delete_trader', $Traders->id) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this trader?')">Delete</button>
               </form>
             </td>
          </tr>
           @endforeach
